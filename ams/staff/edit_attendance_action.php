				...
				<div class="container">
					<div class="navbar-header">
						...
						<div class="navbar-brand center">
							...
							RVCE ATTENDANCE MANAGEMENT SYSTEM
						</div>
						<!-- end: LOGO -->
					</div>
					  ...
					  <div class="toolbar row">
							<div class="col-sm-4 hidden-xs">
								<div class="page-header">
									<h1>Attendance Details <small>Details of Edited Attendance </small></h1>
								</div>
							</div>
									...
									<div class="panel-body">
										<?php
											if(mysqli_num_rows($result)==0)
											{
												...
												echo '</h3>';
												echo '<h3 class = "center"><a class="button center" href="edit_attendance.php">Back to Edit Attendance page</a></h3>';
											}
											else
											{
												$row = mysqli_fetch_array($result);
												$sub = $row['sub_id'];
												$q = 'select * from attendance where sub_id = "'.$sub.'" and acad_year = "'.$year.'" and semester = "'.$sem.'" and semester_type = "'.$sem_type.'" and class_date = "'.$date.'" and class_num = "'.$class_num .'" ';
												$r = mysqli_query($con,$q);
												if(mysqli_num_rows($r)==0)
												{
													echo '<h3 class="center">';
													echo 'No attendance has been marked for this class.';
													echo '</h3>';
													echo '<h3 class = "center"><a class="button center" href="edit_attendance.php">Back to Edit Attendance page</a></h3>';
												...
												else
												{
													...
													$i = 0;
													$flag = 0;
													foreach($txt as $key => $n ) {
														if($n=="")
														{
															continue;
														}
														$q = 'select * from student where usn = "'.$n.'"';
														$r = mysqli_query($con,$q);
														...
														else
														{
															...
															$name = $row1['name'];
															$sql = 'select * from student_subject where usn = "'.$n.'" and sub_id = "'.$sub.'" and acad_year = "'.$year.'" and semester = "'.$sem.'" and semester_type = "'.$sem_type.'"';
															...
															else
															{
																if($flag==0)
																{
																	echo '<h3 class="center">Attendance has been edited.</h3>';
																...
																$sql = 'insert into remarks values("'.$n.'","'.$name.'","'.$sub.'",'.$sem.',"'.$sem_type.'","'.$year.'","'.$date.'","'.$class_num.'","'. $remarks .'")';
		      													$res = mysqli_query($con,$sql);
		      													$sql = 'update attendance set status = "1" where sub_id = "'. $sub .'" and acad_year = "'.$year.'" and semester = "'.$sem.'" and semester_type = "'.$sem_type.'" and class_date = "'.$date.'" and class_num = "'.$class_num .'" ';
		      													$res = mysqli_query($con,$sql);
		      													echo "<tr>";
		      													echo "<td> $n</td>";
																echo "<td> $name</td>";
      												...
      												{
      													echo '<br/>';
      													echo '<h3 class="center">Invalid USN or student not registered for the subject.</h3>';
      													echo '<table class="table table-striped table-bordered table-hover table-full-width"> ';
														echo '<thead><tr>';
															echo '<th>Sl. No.</th>';
															echo '<th>USN</th>';
