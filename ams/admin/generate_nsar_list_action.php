						...
						<div class="toolbar row">
							<div class="col-sm-4 hidden-xs">
								<div class="page-header">
									<h1>NSAR List<small>NSAR Report</small></h1>
								</div>
							</div>
									...
									<div class="panel-body">
										<div class="col-md-12 space20">
												<?php 
													...
													//echo 'Academic Year: ';
													$temp = ($_POST['acad'] + 1)%100;
													/*if($temp>9) echo $_POST['acad'].'-'.$temp;
													else echo $_POST['acad'].'-0'.$temp;
													echo '<br>Semester: '.$_POST['sem'];*/
												?>
											<!--<div class="btn-group pull-right">
												...
												<ul class="dropdown-menu dropdown-light pull-right">
														...
														<li>
															<a href="#" class="export-excel" data-table="#sample_1">
																Export to Excel
															</a>
														</li>
												</ul>
										...
										<?php
											$acad = $_POST['acad'];
										    $sem = $_POST['sem'];
										    $semtype = $_POST['sem_type'];
										    $startdate = $_POST['startdate'];
										    $enddate = $_POST['enddate'];
											$con = mysqli_connect("localhost","root","","attendance");
											$q1 = 'SELECT DISTINCT usn, name from attendance where acad_year = "'.$acad.'" AND semester = "'.$sem.'" AND semester_type = "'.$semtype.'" ';
											$r = mysqli_query($con,$q1);
											if(mysqli_num_rows($r)==0)
											{
												echo'<h2><center>No students registered for this semester.<center></h2>';
												echo '<h3 class = "center"><a class="button center" href="generate_nsar_list.php">Back to Generate NSAR List page</a></h3>';
											...
											else
											{
												...
												/*	echo '<div class = "col-sm-4">';
													echo 'Subject: ';
													$name = $_POST['subid'];
												$sq1 = 'SELECT DISTINCT name from subject where sub_id like "'.$name.'" ';
												$sr = mysqli_query($con,$sq1);

												while($row = mysqli_fetch_array($sr))echo ' '.$row['name']; 
													echo '</div>';	*/
												...
												echo'<button data-toggle="dropdown" class="btn btn-green dropdown-toggle">';
													echo'Export <i class="fa fa-angle-down"></i>';
												echo'</button><br><br><br>';
												echo'<ul class="dropdown-menu dropdown-light pull-right">';
														echo'<li>';
															echo'<a href="#" class="export-png" data-table="#sample_1">';
																echo'Save as PNG';
															echo'</a>';
														echo'</li>';
														echo'<li>';
															echo'<a href="#" class="export-excel" data-table="#sample_1">';
											...
											$q3 = 'SELECT DISTINCT sub_id from attendance where acad_year = "'.$acad.'" AND semester = "'.$sem.'" AND semester_type = "'.$semtype.'" ';
											$r2 = mysqli_query($con,$q3);
												...
												echo '<thead>';
													echo '<tr>';
														echo '<th>Sl No.</th>';
														echo '<th>USN</th>';
														echo '<th class="hidden-xs">Student Name</th>';
												...
												while($res = mysqli_fetch_array($r))
												{
													...
													$name = $res[1];
													echo "<tr>";
													echo '<td class="hidden-xs">' . $z++ . '</td>';
												  	echo '<td class="hidden-xs">' . $res['usn'] . "</td>";
												  	...
												  	while($x = mysqli_fetch_array($r2))
													{
														...
														$att = 0;
														$subid = $x[0];
														$q2 = 'SELECT COUNT(status) as Held, SUM(status) as Attended FROM attendance where usn = "'.$usn.'" AND sub_id="'.$subid.'" AND acad_year = "'.$acad.'" AND semester = "'.$sem.'" AND semester_type = "'.$semtype.'" AND class_date >= "'.$startdate.'" AND class_date <= "'.$enddate.'"';
														//echo '<br/>' . $q2 . '<br/>';
														...
														if($held!=0)
														{
															...
															{
																$p=sprintf("%2.2f",$per);
																echo '<td class="hidden-xs">'. $p .'</td>';

																$_SESSION['email'][$num] = $usn;
																++$num;
																$_SESSION['total']=$num;
															}
															else
											...
											//while( $a = mysqli_fetch_array($r2)) echo $a[0];
											/*echo '<table class="table table-striped table-bordered table-hover table-full-width" id="sample_1">';
